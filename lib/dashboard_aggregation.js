...
// Difficulty cache to avoid repeated queries
let difficultyCache = {
  blockIndex: -1,
  difficulty: 0,
  validUntil: -1  // Block height this difficulty is valid until
};
...
module.exports = {
  ...
  processNewBlock: function(blockHeight, blockHash, timestamp, cb) {
    ...
    Tx.find({ blockindex: blockHeight }).exec().then((txs) => {
      ...
      self.getBlockStats(blockHeight - 1, function(prevBlock) {
        ...
        // Check if we can use cached difficulty
        if (difficultyCache.blockIndex >= 0 && 
            blockHeight >= difficultyCache.blockIndex && 
            blockHeight <= difficultyCache.validUntil) {
          // Use cached difficulty
          const difficulty = difficultyCache.difficulty;
          self.saveBlockStats(blockHeight, blockHash, timestamp, blockInterval, 
                              txCount, blockSize, totalFees, difficulty, cb);
        } else {
          // Get difficulty from networkhistories collection
          const NetworkHistory = require('../models/networkhistory');
          NetworkHistory.find({ blockindex: { $lte: blockHeight } })
            .sort({ blockindex: -1 })
            .limit(2)  // Get current and next to determine valid range
            .exec()
            .then((nethists) => {
              let difficulty = 0;
              if (nethists && nethists.length > 0) {
                const current = nethists[0];
                difficulty = current.difficulty_pow || current.difficulty_pos || 0;
                
                // Cache the difficulty
                difficultyCache.blockIndex = current.blockindex;
                difficultyCache.difficulty = difficulty;
                // Assume difficulty valid until next nethist or +1000 blocks
                if (nethists.length > 1) {
                  difficultyCache.validUntil = nethists[1].blockindex - 1;
                } else {
                  difficultyCache.validUntil = current.blockindex + 1000;
                }
              }
  ...
  // Helper function to save block stats
  saveBlockStats: function(blockHeight, blockHash, timestamp, blockInterval, txCount, blockSize, totalFees, difficulty, cb) {
    const self = this;

const DashboardBlockStats = require('../models/dashboard_block_stats');
const DashboardDailyStats = require('../models/dashboard_daily_stats');
const Tx = require('../models/tx');
const NetworkHistory = require('../models/networkhistory');
const Stats = require('../models/stats');
const settings = require('./settings');

module.exports = {

  buildDailyStatsMapFromBlockStats: function(blockStatsArray) {
    const dailyStatsMap = new Map();

    for (let i = 0; i < blockStatsArray.length; i++) {
      const block = blockStatsArray[i];
      const date = new Date(block.time * 1000).toISOString().split('T')[0];

      if (!dailyStatsMap.has(date)) {
        dailyStatsMap.set(date, {
          blocks: 0,
          tx_count_total: 0,
          block_intervals: [],
          fees_total: 0,
          block_reward_total: 0,
          tx_value_total: 0
        });
      }

      const dailyStats = dailyStatsMap.get(date);
      dailyStats.blocks++;
      dailyStats.tx_count_total += (block.tx_count || 0);
      dailyStats.fees_total += ((block.fees || 0) / 100000000);
      dailyStats.block_reward_total += ((block.block_reward || 0) / 100000000);
      dailyStats.tx_value_total += ((block.tx_value || 0) / 100000000);

      if ((block.block_interval || 0) > 0) {
        dailyStats.block_intervals.push(block.block_interval);
      }
    }

    return dailyStatsMap;
  },
  
  /**
   * Bulk process blocks using MongoDB aggregation
   * Much faster than individual processing
   */
  bulkProcessBlocks: function(startHeight, endHeight, cb) {
    const self = this;
    const startTime = Date.now();
    
    console.log(`[BULK] Processing blocks ${startHeight}-${endHeight}...`);
    
    // Step 1: Use aggregation to calculate all block stats at once
    const aggregationStart = Date.now();
    Tx.aggregate([
      {
        $match: {
          blockindex: { $gte: startHeight, $lte: endHeight }
        }
      },
      {
        $addFields: {
          // Identify coinbase transactions
          is_coinbase: {
            $or: [
              { $eq: [{ $size: { $ifNull: ['$vin', []] } }, 0] },
              { $eq: [{ $arrayElemAt: ['$vin.addresses', 0] }, 'coinbase'] }
            ]
          },
          // Calculate vin total (sum of all input amounts in satoshi)
          vin_total: { $sum: '$vin.amount' },
          // Vout total (total field is already in satoshi)
          vout_total: '$total'
        }
      },
      {
        $addFields: {
          // Fee = vin_total - vout_total (only for non-coinbase tx)
          tx_fee: {
            $cond: [
              '$is_coinbase',
              0,
              { $subtract: ['$vin_total', '$vout_total'] }
            ]
          }
        }
      },
      {
        $group: {
          _id: '$blockindex',
          hash: { $first: '$blockhash' },
          time: { $first: '$timestamp' },
          // Only count non-coinbase transactions
          tx_count: {
            $sum: {
              $cond: ['$is_coinbase', 0, 1]
            }
          },
          total_fees: { $sum: '$tx_fee' },
          // Sum of all non-coinbase transaction values
          total_tx_value: {
            $sum: {
              $cond: ['$is_coinbase', 0, '$vout_total']
            }
          },
          // Coinbase transaction value (block reward + fees)
          coinbase_value: {
            $sum: {
              $cond: ['$is_coinbase', '$vout_total', 0]
            }
          }
        }
      },
      {
        $addFields: {
          // Block reward = coinbase value - fees
          block_reward: { $subtract: ['$coinbase_value', '$total_fees'] }
        }
      },
      {
        $sort: { _id: 1 }
      }
    ]).exec().then((blockAggregates) => {
      const aggregationTime = Date.now() - aggregationStart;
      console.log(`[BENCH] Aggregation completed in ${aggregationTime}ms for ${blockAggregates.length} blocks`);
      
      if (!blockAggregates || blockAggregates.length === 0) {
        return cb(false, 0);
      }

      // Step 2: Get difficulty data in bulk
      const difficultyStart = Date.now();
      NetworkHistory.find({
        blockindex: { $lte: endHeight }
      })
      .sort({ blockindex: -1 })
      .limit(100)  // Get last 100 nethist entries for difficulty lookup
      .exec()
      .then((nethists) => {
        const difficultyTime = Date.now() - difficultyStart;
        console.log(`[BENCH] Difficulty lookup in ${difficultyTime}ms`);
        
        // Create difficulty lookup map
        const difficultyMap = new Map();
        nethists.forEach(nh => {
          difficultyMap.set(nh.blockindex, nh.difficulty_pow || nh.difficulty_pos || 0);
        });

        // Step 2.5: Get last processed block to calculate interval for first block in this batch
        DashboardBlockStats.findOne({ height: startHeight - 1 }).then((lastBlock) => {
          const lastProcessedTime = lastBlock ? lastBlock.time : null;

          // Step 3: Build block stats documents
          const buildStart = Date.now();
          const blockStatsArray = [];
          const dailyStatsMap = new Map();
          
          for (let i = 0; i < blockAggregates.length; i++) {
            const block = blockAggregates[i];
            const height = block._id;
            
            // Calculate block interval
            let blockInterval = 0;
            if (i > 0) {
              blockInterval = block.time - blockAggregates[i - 1].time;
            } else if (i === 0 && lastProcessedTime !== null) {
              // First block in batch: calculate interval from last processed block
              blockInterval = block.time - lastProcessedTime;
            }
          
          // Find closest difficulty
          let difficulty = 0;
          for (const [nethHeight, nethDiff] of difficultyMap) {
            if (nethHeight <= height) {
              difficulty = nethDiff;
              break;
            }
          }
          
          // Create block stats document
          blockStatsArray.push({
            height: height,
            time: block.time,
            block_interval: blockInterval,
            tx_count: block.tx_count,
            block_size: 0,
            fees: block.total_fees || 0,
            block_reward: block.block_reward || 0,
            tx_value: block.total_tx_value || 0,
            difficulty: difficulty,
            hash: block.hash
          });
          
          // Aggregate daily stats
          const date = new Date(block.time * 1000).toISOString().split('T')[0];
          if (!dailyStatsMap.has(date)) {
            dailyStatsMap.set(date, {
              blocks: 0,
              tx_count_total: 0,
              block_intervals: [],
              fees_total: 0,
              block_reward_total: 0,
              tx_value_total: 0
            });
          }
          const dailyStats = dailyStatsMap.get(date);
          dailyStats.blocks++;
          dailyStats.tx_count_total += block.tx_count;
          // Convert fees, rewards, and tx values from satoshi to RIN for daily stats
          dailyStats.fees_total += ((block.total_fees || 0) / 100000000);
          dailyStats.block_reward_total += ((block.block_reward || 0) / 100000000);
          dailyStats.tx_value_total += ((block.total_tx_value || 0) / 100000000);
          if (blockInterval > 0) {
            dailyStats.block_intervals.push(blockInterval);
          }
        }
        
        const buildTime = Date.now() - buildStart;
        console.log(`[BENCH] Document building in ${buildTime}ms`);

        // Step 4: Bulk insert block stats
        const insertStart = Date.now();
        DashboardBlockStats.insertMany(blockStatsArray, { ordered: false })
          .then(() => {
            const insertTime = Date.now() - insertStart;
            console.log(`[BENCH] Bulk insert ${blockStatsArray.length} blocks in ${insertTime}ms`);
            
            // Step 5: Update daily stats
            const dailyStart = Date.now();
            self.bulkUpdateDailyStats(dailyStatsMap, function(success) {
              const dailyTime = Date.now() - dailyStart;
              console.log(`[BENCH] Daily stats update in ${dailyTime}ms`);
              
              const totalTime = Date.now() - startTime;
              const blocksPerSec = (blockStatsArray.length / (totalTime / 1000)).toFixed(2);
              console.log(`[BENCH] TOTAL: ${totalTime}ms for ${blockStatsArray.length} blocks (${blocksPerSec} blocks/sec)`);
              
              return cb(true, blockStatsArray.length);
            });
          })
          .catch((err) => {
            // Ignore duplicate key errors (blocks already processed)
            if (err.code === 11000 && err.writeErrors) {
              const duplicateIndexes = new Set(err.writeErrors.map((writeErr) => writeErr.index));
              const insertedBlocks = blockStatsArray.filter((_, index) => !duplicateIndexes.has(index));
              const inserted = insertedBlocks.length;
              console.log(`[BULK] Inserted ${inserted} blocks (${err.writeErrors.length} duplicates skipped)`);

              if (inserted === 0) {
                const totalTime = Date.now() - startTime;
                console.log(`[BENCH] TOTAL: ${totalTime}ms`);
                return cb(true, 0);
              }

              const insertedDailyStatsMap = self.buildDailyStatsMapFromBlockStats(insertedBlocks);

              self.bulkUpdateDailyStats(insertedDailyStatsMap, function(success) {
                const totalTime = Date.now() - startTime;
                console.log(`[BENCH] TOTAL: ${totalTime}ms`);
                return cb(true, inserted);
              });
            } else {
              console.error('[BULK] Insert error:', err.message);
              return cb(false, 0);
            }
          });
        }).catch((err) => {
          console.error('[BULK] Error looking up last processed block:', err);
          return cb(false, 0);
        });
      })
      .catch((err) => {
        console.error('[BULK] Difficulty lookup error:', err);
        return cb(false, 0);
      });
    }).catch((err) => {
      console.error('[BULK] Aggregation error:', err);
      return cb(false, 0);
    });
  },

  /**
   * Bulk update daily stats using aggregation
   */
  bulkUpdateDailyStats: function(dailyStatsMap, cb) {
    const self = this;
    
    // First, fetch existing documents to calculate proper weighted averages
    const dates = Array.from(dailyStatsMap.keys());
    DashboardDailyStats.find({ date: { $in: dates } })
      .then((existingDocs) => {
        const existingMap = new Map();
        existingDocs.forEach(doc => {
          existingMap.set(doc.date, doc);
        });
        
        const bulkOps = [];
        
        for (const [date, stats] of dailyStatsMap) {
          const existing = existingMap.get(date);
          let newAvgBlockTime;
          
          if (stats.block_intervals.length > 0) {
            const batchAvg = stats.block_intervals.reduce((a, b) => a + b, 0) / stats.block_intervals.length;
            
            if (existing && existing.blocks > 0) {
              // Weighted average: (old_avg * old_count + new_total) / (old_count + new_count)
              const oldTotal = existing.avg_block_time * existing.blocks;
              const newTotal = batchAvg * stats.block_intervals.length;
              newAvgBlockTime = (oldTotal + newTotal) / (existing.blocks + stats.blocks);
            } else {
              // First batch for this date
              newAvgBlockTime = batchAvg;
            }
          } else {
            // No new block intervals, keep existing or use 0
            newAvgBlockTime = existing ? existing.avg_block_time : 0;
          }
          
          bulkOps.push({
            updateOne: {
              filter: { date: date },
              update: {
                $inc: {
                  blocks: stats.blocks,
                  tx_count_total: stats.tx_count_total,
                  fees_total: stats.fees_total,
                  block_reward_total: stats.block_reward_total,
                  tx_value_total: stats.tx_value_total
                },
                $set: {
                  avg_block_time: newAvgBlockTime,
                  avg_block_size: 0,
                  issuance: 0
                }
              },
              upsert: true
            }
          });
        }
        
        if (bulkOps.length === 0) {
          return cb(true);
        }
        
        DashboardDailyStats.bulkWrite(bulkOps)
          .then(() => {
            return cb(true);
          })
          .catch((err) => {
            console.error('[BULK] Daily stats error:', err);
            return cb(false);
          });
      })
      .catch((err) => {
        console.error('[BULK] Error fetching existing daily stats:', err);
        return cb(false);
      });
  },

  /**
   * Initialize rolling cache from database
   */
  initializeRollingCache: function(currentHeight, cb) {
    DashboardBlockStats.find({ height: { $gte: currentHeight - 1500, $lte: currentHeight } })
      .sort({ height: 1 })
      .exec()
      .then((blocks) => {
        console.log(`[CACHE] Initialized with ${blocks.length} blocks`);
        return cb(true);
      })
      .catch((err) => {
        console.error('[CACHE] Initialization error:', err);
        return cb(false);
      });
  },

  /**
   * Get dashboard data for display
   * @param {number|string} days - Number of days to fetch, or 'all' for all history
   * @param {function} cb - Callback function
   */
  getDashboardData: function(days, cb) {
    // Handle legacy single-parameter calls
    if (typeof days === 'function') {
      cb = days;
      days = 30; // Default to 30 days
    }
    
    days = days || 30;
    Stats.findOne({ coin: (settings.coin.dbname || settings.coin.name) }).then((stats) => {
      if (!stats) {
        return cb(null);
      }

      // Get rolling averages using aggregation
      const promises = [
        this.getRollingAveragesAggregated(stats.last, 60),
        this.getRollingAveragesAggregated(stats.last, 1440),
        this.getRollingAveragesAggregated(stats.last, 10000)
      ];

      Promise.all(promises).then(([avg60, avg1440, avg10000]) => {
        // Get daily stats based on requested time range
        const today = new Date();
        const endDate = today.toISOString().split('T')[0];
        
        let query;
        if (days === 'all') {
          query = {}; // No date filter = all history
        } else {
          const daysAgo = new Date(today.getTime() - (days * 24 * 60 * 60 * 1000));
          const startDate = daysAgo.toISOString().split('T')[0];
          query = { date: { $gte: startDate, $lte: endDate } };
        }

        DashboardDailyStats.find(query)
          .sort({ date: 1 })
          .exec()
          .then((dailyStats) => {
            // Get latest block stats
            DashboardBlockStats.findOne().sort({ height: -1 }).exec().then((latestBlock) => {
              const dashboardData = {
                current: {
                  latestBlockHeight: stats.last,
                  latestBlockHash: latestBlock ? latestBlock.hash : '',
                  difficulty: latestBlock ? latestBlock.difficulty : 0,
                  totalSupply: stats.supply,
                  totalTxCount: stats.txes
                },
                rolling: {
                  last60: avg60,
                  last1440: avg1440,
                  last10000: avg10000
                },
                daily: dailyStats
              };

              return cb(dashboardData);
            });
          });
      }).catch((err) => {
        console.error('Error getting dashboard data:', err);
        return cb(null);
      });
    }).catch((err) => {
      console.error('Error getting stats:', err);
      return cb(null);
    });
  },

  /**
   * Get rolling averages using aggregation
   */
  getRollingAveragesAggregated: function(currentHeight, windowSize) {
    return DashboardBlockStats.aggregate([
      {
        $match: {
          height: { $gt: currentHeight - windowSize, $lte: currentHeight }
        }
      },
      {
        $group: {
          _id: null,
          blockInterval: { $avg: '$block_interval' },
          txCount: { $avg: '$tx_count' },
          txCountSum: { $sum: '$tx_count' },
          txValueSum: { $sum: '$tx_value' },
          blockSize: { $avg: '$block_size' },
          fees: { $avg: '$fees' },
          feesSum: { $sum: '$fees' }
        }
      }
    ]).exec().then((result) => {
      if (result && result.length > 0) {
        return result[0];
      }
      return {
        blockInterval: 0,
        txCount: 0,
        txCountSum: 0,
        txValueSum: 0,
        blockSize: 0,
        fees: 0,
        feesSum: 0
      };
    });
  }
};

...
module.exports = {
  ...
  get_networkinfo: function(cb) {
    rpcCommand([{method: "getnetworkinfo", parameters: []}], function(response) {
      if (!response || typeof response !== 'object') {
        console.log("Ncb(null)");
        return cb(null);
      }
      console.log("Ncb(response)");
      return cb(response);
    });
  },
  ...
  get_geo_location: function(address, cb) {
    request({
      uri: 'https://reallyfreegeoip.org/json/' + address,
      json: true,
      timeout: 10000
      ...
      if (response == null || response.statusCode == null || response.statusCode < 200 || response.statusCode >= 300)
        return cb(new Error(`Geo lookup failed with status ${(response && response.statusCode ? response.statusCode : 'unknown')}`), null);

      if (geo == null || typeof geo != 'object')
        return cb(new Error('Geo lookup returned invalid data'), null);
  ...
  get_market_currency_code: function() {
    ...
    if (settings.markets_page.market_price == 'COINGECKO' || settings.markets_page.market_price == 'COINPAPRIKA') {

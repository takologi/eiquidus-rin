    ...
    Stats.findOne({ coin: coin }).then((stats) => {
      ...
      function get_history_difficulty(block_stats, cb_diff) {
        if (block_stats != null && block_stats.difficulty != null && block_stats.difficulty > 0)
          return cb_diff(block_stats.difficulty);
        ...
        NetworkHistory.findOne({ blockindex: { $lte: effective_height } }).sort({ blockindex: -1 }).exec().then((network_history) => {
          ...
          if (settings.shared_pages.difficulty == 'POW')
            return cb_diff((network_history.difficulty_pow ? network_history.difficulty_pow : 0));
          else if (settings.shared_pages.difficulty == 'POS')
            return cb_diff((network_history.difficulty_pos ? network_history.difficulty_pos : 0));
          else {
            const pow_value = (network_history.difficulty_pow ? network_history.difficulty_pow : 0);
            const pos_value = (network_history.difficulty_pos ? network_history.difficulty_pos : 0);

            return cb_diff((pow_value > 0 ? pow_value : pos_value));
          }
        }).catch((err) => {
          console.log(err);
          return cb_diff(0);

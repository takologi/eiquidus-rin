    ...
    this.get_last_txs_ajax(start, length, min, order_col, order_dir, function(txs, count) {
      ...
      async.timesSeries(txs.length, function(i, loop) {
        ...
        if (internal && data.length > 0) {
          const uniqueIndexes = [...new Set(data.map(function(r) { return r[0]; }))];

          Orphans.find({blockindex: {$in: uniqueIndexes}}).select('blockindex reorg_depth').lean().exec().then(function(orphanRecords) {
            const depthMap = {};

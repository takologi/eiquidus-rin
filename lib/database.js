...
function get_coin_db_name(coin) {
  return settings.coin.dbname || coin || settings.coin.name;
}
...
module.exports = {
  ...
  update_richlist_claim_name: function(hash, claim_name, cb) {
    ...
    if (settings.richlist_page.enabled == true) {
      ...
      module.exports.get_richlist(get_coin_db_name(), function(richlist) {
        var updated = false;
        ...
        if (updated) {
          ...
          Richlist.updateOne({coin: get_coin_db_name()}, {
  ...
  get_last_txs: function(start, length, min, internal, order_col, order_dir, cb) {
    this.get_last_txs_ajax(start, length, min, order_col, order_dir, function(txs, count) {
      ...
      async.timesSeries(txs.length, function(i, loop) {
        ...
        // for internal calls, annotate each row with orphan depth (last element)
        if (internal && data.length > 0) {
          // build the set of blockindexes that have has_reorg=true — already loaded in txs, no extra query
          const reorgBlockIndexes = new Set();

          txs.forEach(function(t) {
            if (t.has_reorg)
              reorgBlockIndexes.add(t.blockindex);
          });

          // fast path: no reorged blocks on this page
          if (reorgBlockIndexes.size === 0) {
            data.forEach(function(row) { row.push('orphan_depth:0'); });
            return cb(data, count);
          }

          // slow path: look up depth only for the blocks that have reorgs
          Orphans.find({blockindex: {$in: [...reorgBlockIndexes]}}).select('blockindex reorg_depth').lean().exec().then(function(orphanRecords) {
            const depthMap = {};

            (orphanRecords || []).forEach(function(rec) {
              const idx = rec.blockindex;
              const d = (rec.reorg_depth || 1);

              if (depthMap[idx] == null || d > depthMap[idx])
                depthMap[idx] = d;
            });

            data.forEach(function(row) {
              row.push('orphan_depth:' + (depthMap[row[0]] || 0));
            });

            return cb(data, count);
          }).catch(function(err) {
            // orphan annotation failure is non-fatal; return data without depth
            console.log('get_last_txs orphan annotation error: ' + err);
            data.forEach(function(row) { row.push('orphan_depth:0'); });
            return cb(data, count);
          });
        } else {
  ...
  get_last_txs_ajax: function(start, length, min, order_col, order_dir, cb) {
    const timestamp_col = 5 + (settings.block_page.multi_algorithm.show_algo == true ? 1 : 0) + (settings.index_page.show_extracted_by == true ? 1 : 0);

    let primary_sort_field = 'blockindex';
    switch (Number(order_col)) {
      case 1:
        primary_sort_field = 'blockindex';
        break;
      case 2:
        primary_sort_field = 'txid';
        break;
      case 3:
        primary_sort_field = 'recipients_count';
        break;
      case 4:
        primary_sort_field = 'total';
        break;
      case timestamp_col:
        primary_sort_field = 'timestamp';
        break;
      default:
        primary_sort_field = 'blockindex';
        break;
    ...
    const sort_direction = (order_dir != null && order_dir.toString().toLowerCase() == 'asc' ? 1 : -1);
    let sort_pipeline = {};

    if (primary_sort_field == 'recipients_count') {
      sort_pipeline.recipients_count = sort_direction;
      sort_pipeline.timestamp = sort_direction;
    } else if (primary_sort_field == 'timestamp') {
      sort_pipeline.timestamp = sort_direction;
      sort_pipeline.blockindex = sort_direction;
    } else {
      sort_pipeline[primary_sort_field] = sort_direction;
      sort_pipeline.timestamp = sort_direction;
    }

    // Build the aggregate pipeline once and reuse it for both count branches.
    // Using 'vout.0': {$exists: true} in the first (and only) $match stage instead of a
    // separate $addFields + $match keeps the pipeline index-friendly: MongoDB can use the
    // blockindex / timestamp index for the $sort stage and apply $skip in O(1) instead of
    // scanning the full collection into memory.  When sorting by recipients_count we still
    // need $addFields (computed sort field), but it is placed after the initial $match so
    // the filter benefit is preserved.
    const agg_pipeline = primary_sort_field === 'recipients_count'
      ? [
          { $match: { total: {$gte: min}, 'vout.0': {$exists: true} } },
          { $addFields: { recipients_count: { $size: { $ifNull: ['$vout', []] } } } },
          { $sort: sort_pipeline },
          { $skip: Number(start) },
          { $limit: Number(length) }
        ]
    ...
    if (min > 0) {
      ...
      Tx.find({'total': {$gte: min}}).countDocuments().then((count) => {
        ...
        Tx.aggregate(agg_pipeline).then((txs) => {
          return cb(txs, count);
        }).catch((err) => {
          return cb(err);
        });
      ...
      Stats.findOne({coin: get_coin_db_name()}).then((stats) => {
        // Get last transactions where there is at least 1 vout
        Tx.aggregate(agg_pipeline).then((txs) => {
  ...
  get_address_txs_ajax: function(hash, start, length, history_block, cb) {
    if (typeof history_block === 'function') {
      cb = history_block;
      history_block = null;
    }

    let address_filter = {a_id: hash};

    if (history_block != null && !isNaN(history_block))
      address_filter.blockindex = {$lte: Number(history_block)};

    AddressTx.find(address_filter).countDocuments().then((totalCount) => {
      AddressTx.aggregate([
        ...
        AddressTx.find(address_filter).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).exec().then((address_tx) => {
          let txs = [];
  ...
  get_address_balance_at_block: function(hash, history_block, cb) {
    if (history_block == null || isNaN(history_block))
      return cb(null);

    const target_height = Number(history_block);
    ...
    HistoryCheckpoint.findOne({ height: { $lte: target_height } }).sort({ height: -1 }).select('height').lean().then((checkpoint) => {
      if (!checkpoint || checkpoint.height == null)
        return run_legacy();

      HistoryWalletState.findOne({ checkpoint_height: Number(checkpoint.height), a_id: hash }).select('balance').lean().then((state) => {
        const base_balance = (state && state.balance != null ? Number(state.balance) : 0);

        if (Number(checkpoint.height) >= target_height)
          return cb(base_balance);

        AddressTx.aggregate([
          {
            $match: {
              a_id: hash,
              blockindex: { $gt: Number(checkpoint.height), $lte: target_height }
            }
          },
          {
            $group: {
              _id: null,
              delta_balance: { $sum: '$amount' }
            }
          }
        ]).then((delta_rows) => {
          const delta_balance = (delta_rows && delta_rows.length > 0 ? Number(delta_rows[0].delta_balance || 0) : 0);
          return cb(base_balance + delta_balance);
        }).catch((err) => {
          console.log(err);
          return run_legacy();
        });
  ...
  get_address_summary_at_block: function(hash, history_block, cb) {
    if (history_block == null || isNaN(history_block))
      return cb(null);
    ...
    AddressTx.find(address_filter).select('txid amount').exec().then((address_txs) => {
      if (!address_txs || address_txs.length == 0) {
        return cb({
          balance: 0,
          sent: 0,
          received: 0
      ...
      const txid_list = [...new Set(address_txs.map(item => item.txid))];

      Tx.find({txid: {$in: txid_list}}).select('txid vin vout').exec().then((txs) => {
        const tx_map = {};
        let balance = 0;
        let sent = 0;
        let received = 0;

        txs.forEach(function(current_tx) {
          tx_map[current_tx.txid] = current_tx;
        });

        address_txs.forEach(function(current_address_tx) {
          const tx = tx_map[current_address_tx.txid];

          // net balance from AddressTx records
          balance += current_address_tx.amount;

          if (tx && tx.vout && tx.vin) {
            tx.vout.forEach(function(vout_entry) {
              if (vout_entry.addresses == hash)
                received += vout_entry.amount;
            });

            tx.vin.forEach(function(vin_entry) {
              if (vin_entry.addresses == hash)
                sent += vin_entry.amount;
            });
          }
        });
    ...
    HistoryCheckpoint.findOne({ height: { $lte: target_height } }).sort({ height: -1 }).select('height').lean().then((checkpoint) => {
      ...
      HistoryWalletState.findOne({ checkpoint_height: Number(checkpoint.height), a_id: hash }).select('balance sent received').lean().then((state) => {
        const base_balance = (state && state.balance != null ? Number(state.balance) : 0);
        const base_sent = (state && state.sent != null ? Number(state.sent) : 0);
        const base_received = (state && state.received != null ? Number(state.received) : 0);
        ...
        AddressTx.aggregate([
          ...
          const delta = (delta_rows && delta_rows.length > 0 ? delta_rows[0] : null);

          return cb({
            balance: (base_balance + Number(delta != null && delta.delta_balance != null ? delta.delta_balance : 0)),
            sent: (base_sent + Number(delta != null && delta.delta_withdrawn != null ? delta.delta_withdrawn : 0)),
            received: (base_received + Number(delta != null && delta.delta_deposited != null ? delta.delta_deposited : 0))
          });
        }).catch((err) => {
          console.log(err);
          return run_legacy();
  ...
  get_history_browser_overview: function(history_block, cb) {
    const coin = get_coin_db_name();

    Stats.findOne({ coin: coin }).then((stats) => {
      if (!stats)
        return cb(null);

      const requested_history = (history_block != null && !isNaN(history_block) ? Number(history_block) : null);
      const has_history = (requested_history != null && requested_history >= 0 && requested_history <= stats.last);
      const effective_height = (has_history ? requested_history : stats.last);

      function get_history_difficulty(block_stats, cb_diff) {
        function pick_difficulty(pow_value, pos_value, fallback_value) {
          const pow = (pow_value != null && !isNaN(pow_value) ? Number(pow_value) : 0);
          const pos = (pos_value != null && !isNaN(pos_value) ? Number(pos_value) : 0);
          const fallback = (fallback_value != null && !isNaN(fallback_value) ? Number(fallback_value) : 0);

          if (settings.shared_pages.difficulty == 'POW')
            return (pow > 0 ? pow : (pos > 0 ? pos : fallback));
          else if (settings.shared_pages.difficulty == 'POS')
            return (pos > 0 ? pos : (pow > 0 ? pow : fallback));
          else
            return (pow > 0 ? pow : (pos > 0 ? pos : fallback));
        }

        function fallback_to_block_rpc() {
          if (block_stats == null || block_stats.hash == null || block_stats.hash == '')
            return cb_diff(0);

          lib.get_block(block_stats.hash, function(block_data) {
            if (block_data != null && block_data.difficulty != null && !isNaN(block_data.difficulty) && Number(block_data.difficulty) > 0)
              return cb_diff(Number(block_data.difficulty));
            else
              return cb_diff(0);
          });
        }

        if (block_stats != null && block_stats.difficulty != null && block_stats.difficulty > 0)
          return cb_diff(Number(block_stats.difficulty));

        NetworkHistory.findOne({ blockindex: { $lte: effective_height } }).sort({ blockindex: -1 }).exec().then((network_history) => {
          if (!network_history)
            return fallback_to_block_rpc();

          const selected_difficulty = pick_difficulty(network_history.difficulty_pow, network_history.difficulty_pos, block_stats != null ? block_stats.difficulty : 0);

          if (selected_difficulty > 0)
            return cb_diff(selected_difficulty);
          else
            return fallback_to_block_rpc();
        }).catch((err) => {
          console.log(err);
      ...
      DashboardBlockStats.findOne({ height: effective_height }).then((block_stats) => {
        const latest_hash = (block_stats ? block_stats.hash : '');
        const latest_time = (block_stats ? block_stats.time : null);
        get_history_difficulty(block_stats, function(difficulty) {
          if (has_history) {
            HistoryChainState.findOne({ height: { $lte: effective_height } }).sort({ height: -1 }).select('height supply tx_count').lean().then((chain_state) => {
              if (!chain_state || chain_state.height == null) {
                return Promise.all([
                  DashboardBlockStats.aggregate([
                    { $match: { height: { $lte: effective_height } } },
                    { $group: { _id: null, total_reward: { $sum: '$block_reward' } } }
                  ]).exec(),
                  Tx.countDocuments({ blockindex: { $lte: effective_height } })
                ]).then(([reward_data, tx_count]) => {
                  const total_supply = (reward_data != null && reward_data.length > 0 ? (reward_data[0].total_reward / 100000000) : 0);

                  return cb({
                    current_height: stats.last,
                    effective_height: effective_height,
                    is_historical: true,
                    latestBlockHeight: effective_height,
                    latestBlockHash: latest_hash,
                    latestBlockTime: latest_time,
                    difficulty: difficulty,
                    totalSupply: total_supply,
                    totalTxCount: tx_count,
                    lastUpdated: stats.blockchain_last_updated
                  });
                });
              }
                ...
                DashboardBlockStats.aggregate([
                  {
                    $match: {
                      height: {
                        $gt: Number(chain_state.height),
                        $lte: effective_height
                      }
                    }
                  },
                  { $group: { _id: null, total_reward: { $sum: '$block_reward' } } }
              ...
              ]).then(([reward_delta_data, tx_count_delta]) => {
                const reward_delta = (reward_delta_data != null && reward_delta_data.length > 0 ? Number(reward_delta_data[0].total_reward || 0) : 0);
                const total_supply = Number(chain_state.supply || 0) + (reward_delta / 100000000);
                const total_tx_count = Number(chain_state.tx_count || 0) + Number(tx_count_delta || 0);
            ...
            return cb({
              ...
              latestBlockTime: latest_time,
              difficulty: difficulty,
              totalSupply: (stats.supply ? stats.supply : 0),
              totalTxCount: (stats.txes ? stats.txes : 0),
              lastUpdated: stats.blockchain_last_updated
            });
          }
        });
  ...
  get_history_wallets: function(start, length, history_block, order_col, order_dir, cb) {
    const address_match = {
      a_id: { $nin: ['coinbase', 'hidden_address', 'unknown_address'] }
    };

    const sort_direction = (order_dir == 'asc' ? 1 : -1);
    const sort_map = {
      0: 'address_sort',
      1: 'tx_count',
      2: 'deposited',
    ...
    const primary_sort_field = (sort_map[order_col] != null ? sort_map[order_col] : 'balance');

    const wallet_sort = {};
    wallet_sort[primary_sort_field] = sort_direction;
    if (primary_sort_field != 'last_activity_block')
      wallet_sort.last_activity_block = sort_direction;
    if (primary_sort_field != '_id' && primary_sort_field != 'address_sort')
      wallet_sort._id = 1;

    function run_legacy_query(target_height) {
      const legacy_match = {
        a_id: { $nin: ['coinbase', 'hidden_address', 'unknown_address'] }
      };

      if (target_height != null && !isNaN(target_height))
        legacy_match.blockindex = { $lte: Number(target_height) };
      ...
      AddressTx.aggregate([
        ...
        const total_count = (count_data != null && count_data.length > 0 ? count_data[0].total : 0);
        ...
        AddressTx.aggregate([
          ...
          {
            $lookup: {
              from: 'dashboardblockstats',
              let: { to_block_height: '$last_to_block' },
              pipeline: [
                { $match: { $expr: { $eq: ['$height', '$$to_block_height'] } } },
                { $project: { _id: 0, time: 1 } }
              ],
              as: 'last_to_block_data'
            }
          },
          {
            $lookup: {
              from: 'dashboardblockstats',
              let: { from_block_height: '$last_from_block' },
              pipeline: [
                { $match: { $expr: { $eq: ['$height', '$$from_block_height'] } } },
                { $project: { _id: 0, time: 1 } }
              ],
              as: 'last_from_block_data'
            }
          },
          {
            $addFields: {
              ...
              last_activity_block: {
                $cond: [
                  { $gt: ['$last_to_block', '$last_from_block'] },
                  '$last_to_block',
                  '$last_from_block'
                ]
              }
          ...
          { $limit: Number(length) },
          {
            $project: {
              ...
              last_to_timestamp: {
                $ifNull: [
                  { $arrayElemAt: ['$last_to_block_data.time', 0] },
                  0
                ]
              },
              last_from_timestamp: {
                $ifNull: [
                  { $arrayElemAt: ['$last_from_block_data.time', 0] },
                  0
                ]
              }
            }
          ...
          return cb((rows || []), total_count);
        }).catch((err) => {
          console.log(err);
          return cb([], total_count);
        });
      }).catch((err) => {
        console.log(err);
        return cb([], 0);
    ...
    function get_target_height(cb_height) {
      if (history_block != null && !isNaN(history_block))
        return cb_height(Number(history_block));

      Stats.findOne({ coin: get_coin_db_name() }).select('last').lean().then((stats) => {
        return cb_height((stats && stats.last != null ? Number(stats.last) : null));
      }).catch((err) => {
        console.log(err);
        return cb_height(null);
      });
    }

    function enrich_timestamps(rows, cb_rows) {
      const block_heights = [];

      rows.forEach((row) => {
        if (row.last_to_block != null && row.last_to_block > 0)
          block_heights.push(Number(row.last_to_block));
        if (row.last_from_block != null && row.last_from_block > 0)
          block_heights.push(Number(row.last_from_block));
      });

      const unique_heights = [...new Set(block_heights)];

      if (unique_heights.length == 0)
        return cb_rows(rows.map((row) => {
          row.last_to_timestamp = 0;
          row.last_from_timestamp = 0;
          return row;
        }));

      DashboardBlockStats.find({ height: { $in: unique_heights } }).select('height time').lean().then((blocks) => {
        const time_map = {};

        (blocks || []).forEach((entry) => {
          time_map[Number(entry.height)] = (entry.time != null ? Number(entry.time) : 0);
        });

        rows.forEach((row) => {
          row.last_to_timestamp = (row.last_to_block > 0 && time_map[row.last_to_block] != null ? time_map[row.last_to_block] : 0);
          row.last_from_timestamp = (row.last_from_block > 0 && time_map[row.last_from_block] != null ? time_map[row.last_from_block] : 0);
        });
    ...
    function sort_and_page_rows(rows) {
      ...
      rows.forEach((row) => {
        row.address_sort = (row.address_sort != null ? row.address_sort : row._id.toString());
        row.last_activity_block = Math.max(Number(row.last_to_block || 0), Number(row.last_from_block || 0));
      });

      rows.sort((a, b) => {
        let cmp = 0;

        if (primary_sort_field == 'address_sort')
          cmp = compare_string(a.address_sort, b.address_sort);
        else
          cmp = compare_number(a[primary_sort_field], b[primary_sort_field]);

        if (cmp != 0)
          return (sort_direction * cmp);

        if (primary_sort_field != 'last_activity_block') {
          const activity_cmp = compare_number(a.last_activity_block, b.last_activity_block);

          if (activity_cmp != 0)
            return (sort_direction * activity_cmp);
        }

        return compare_string(a._id, b._id);
      });

      const total_count = rows.length;
      const paged_rows = rows.slice(Number(start), (Number(start) + Number(length))).map((row) => {
        return {
          _id: row._id,
          ...
          last_to_block: Number(row.last_to_block || 0),
          last_from_block: Number(row.last_from_block || 0)
        };
      });

      enrich_timestamps(paged_rows, function(final_rows) {
        return cb(final_rows, total_count);
      });
    }

    function run_checkpoint_query(target_height) {
      if (target_height == null || isNaN(target_height))
        return run_legacy_query(history_block);

      const numeric_target = Number(target_height);

      HistoryCheckpoint.findOne({ height: { $lte: numeric_target } }).sort({ height: -1 }).select('height').lean().then((checkpoint) => {
        if (!checkpoint || checkpoint.height == null)
          return run_legacy_query(target_height);

        HistoryWalletState.find({ checkpoint_height: Number(checkpoint.height) }).select('a_id address_sort tx_count deposited balance withdrawn last_to_block last_from_block').lean().then((base_rows) => {
          const row_map = {};

          (base_rows || []).forEach((row) => {
            row_map[row.a_id] = {
              _id: row.a_id,
              address_sort: (row.address_sort != null ? row.address_sort : row.a_id),
              tx_count: Number(row.tx_count || 0),
              deposited: Number(row.deposited || 0),
              balance: Number(row.balance || 0),
              withdrawn: Number(row.withdrawn || 0),
          ...
          function finalize_rows() {
            return sort_and_page_rows(Object.keys(row_map).map((key) => row_map[key]));
          }

          if (Number(checkpoint.height) >= numeric_target)
          ...
          AddressTx.aggregate([
            {
              $match: {
                a_id: { $nin: ['coinbase', 'hidden_address', 'unknown_address'] },
                blockindex: { $gt: Number(checkpoint.height), $lte: numeric_target }
              }
            },
            {
              $group: {
          ...
          ], { allowDiskUse: true }).then((delta_rows) => {
            (delta_rows || []).forEach((delta_row) => {
              if (row_map[delta_row._id] == null) {
                row_map[delta_row._id] = {
                  _id: delta_row._id,
                  address_sort: delta_row._id.toString(),
                  tx_count: 0,
                  deposited: 0,
                  balance: 0,
                  withdrawn: 0,
              ...
              row_map[delta_row._id].tx_count += Number(delta_row.tx_count || 0);
              row_map[delta_row._id].deposited += Number(delta_row.deposited || 0);
              row_map[delta_row._id].balance += Number(delta_row.balance || 0);
              row_map[delta_row._id].withdrawn += Number(delta_row.withdrawn || 0);
              row_map[delta_row._id].last_to_block = Math.max(Number(row_map[delta_row._id].last_to_block || 0), Number(delta_row.last_to_block || 0));
              row_map[delta_row._id].last_from_block = Math.max(Number(row_map[delta_row._id].last_from_block || 0), Number(delta_row.last_from_block || 0));
            });
    ...
    get_target_height(function(target_height) {
      return run_checkpoint_query(target_height);
    });
  },
        ...
        lib.get_vote(function(vote) {
          lib.get_phase(function(phase) {
            lib.get_reward(function(reward) {
              module.exports.get_stats(get_coin_db_name(), function(stats) {
                  ...
                  lib.get_nextin(function(nextin) {
                    ...
                    async.timesSeries(count, function(i, loop) {
                      ...
                      Heavy.updateOne({coin: coin}, {
                        ...
                        module.exports.update_last_updated_stats(get_coin_db_name(), { reward_last_updated: Math.floor(new Date() / 1000) }, function (update_success) {
  ...
  initialize_data_startup: function(cb) {
    ...
    module.exports.check_stats(get_coin_db_name(), function(stats_exists) {
      var skip = true;
      ...
      module.exports.create_stats(get_coin_db_name(), skip, function() {
              ...
              module.exports.check_masternodes(function(masternodes_exists) {
                ...
                module.exports.check_networkhistory(function(networkhistory_exists) {
                  ...
                  module.exports.check_peers(function(peers_exists) {
                    ...
                    module.exports.check_richlist(get_coin_db_name(), function(richlist_exists) {
                      skip = true;
                      ...
                      module.exports.create_richlist(get_coin_db_name(), skip, function() {
                        // check and initialize the heavycoin collection
                        init_heavy(function() {
                          // check and initialize the claimaddress collection
                          init_claimaddress(get_coin_db_name(), function() {
                            // initialize all enabled plugins
                            init_plugins(get_coin_db_name(), function() {
  ...
  // get the last orphaned blocks for public api use
  get_last_orphans: function(start, length, min, cb) {
    const min_depth = (isNaN(min) || Number(min) < 0 ? 0 : Number(min));
    const orphan_filter = {
      reorg_depth: { $gt: min_depth }
    };

    // get the count of orphaned blocks that pass the min reorg depth filter
    Orphans.find(orphan_filter).countDocuments().then((count) => {
      // get the filtered orphaned block data
      Orphans.find(orphan_filter).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).lean().exec().then((orphans) => {
        if (orphans == null || orphans.length == 0)
          return cb([], count);

        // aggregate orphaned transaction totals by orphan block hash for fast amount lookup
        const orphan_blockhashes = orphans.map((o) => o.orphan_blockhash).filter((h) => h != null && h != '');

        if (orphan_blockhashes.length == 0) {
          const empty_amount_data = orphans.map((orphan) => {
            return {
              blockindex: orphan.blockindex,
              blockhash: orphan.orphan_blockhash,
              prev_blockhash: orphan.prev_blockhash,
              reorg_depth: (orphan.reorg_depth || 1),
              tx_count: (orphan.tx_count || 0),
              amount: 0,
              timestamp: (orphan.detected_at ? Math.floor(new Date(orphan.detected_at).getTime() / 1000) : 0)
            };
          });

          return cb(empty_amount_data, count);
        }

        OrphanedTx.aggregate([
          {
            $match: {
              orphan_blockhash: { $in: orphan_blockhashes }
            }
          },
          {
            $group: {
              _id: '$orphan_blockhash',
              amount: { $sum: '$total' },
              timestamp: { $max: '$timestamp' }
            }
          }
        ]).then((orphan_tx_summaries) => {
          const orphan_tx_map = {};

          (orphan_tx_summaries || []).forEach((summary) => {
            orphan_tx_map[summary._id] = {
              amount: (summary.amount || 0),
              timestamp: (summary.timestamp || 0)
            };
          });

          const data = orphans.map((orphan) => {
            const orphan_tx_data = orphan_tx_map[orphan.orphan_blockhash] || { amount: 0, timestamp: 0 };
            const detected_at_timestamp = (orphan.detected_at ? Math.floor(new Date(orphan.detected_at).getTime() / 1000) : 0);
            ...
            return {
              ...
              reorg_depth: (orphan.reorg_depth || 1),
              tx_count: (orphan.tx_count || 0),
              amount: (orphan_tx_data.amount / 100000000),
              timestamp: (detected_at_timestamp || orphan_tx_data.timestamp || 0)
            };
          });
          ...
          const fallback_data = orphans.map((orphan) => {
            return {
              blockindex: orphan.blockindex,
              blockhash: orphan.orphan_blockhash,
              prev_blockhash: orphan.prev_blockhash,
  ...
  // Dashboard functions for Phase 1
  get_dashboard_current_data: function(cb) {
    const self = this;
    
    // Get stats
    self.get_stats(get_coin_db_name(), function(stats) {
      if (!stats) {
        return cb(null);
      }

      // Get last 10 blocks to compute recent metrics
      Tx.aggregate([
        { $match: { blockindex: { $gte: stats.last - 10, $lte: stats.last } } },
        { $group: {
          _id: '$blockindex',
          txCount: { $sum: 1 },
          totalFees: { $sum: '$total' }
        ...
        self.get_last_txs(0, 1, 0, false, function(txs, txCount) {
          if (txs && txs.length > 0) {
            const latestTx = txs[0];
            
            // Calculate block interval from recent blocks
            let avgBlockInterval = 0;
            if (blockData && blockData.length > 1) {
              // We'd need block timestamps; simplified for now
              avgBlockInterval = 60; // Rincoin target is 60s
            }

            // Calculate average tx count and block size
            let avgTxCount = 0;
            let recentTxCount = 0;
            if (blockData && blockData.length > 0) {
              const sum = blockData.reduce((acc, b) => acc + b.txCount, 0);
              avgTxCount = sum / blockData.length;
              recentTxCount = blockData[0].txCount || 0;
            }

            const dashboardData = {
              latestBlockHeight: stats.last,
              latestBlockHash: latestTx.blockhash,
              difficulty: latestTx.difficulty || 0,
              totalSupply: stats.supply,
              totalTxCount: stats.txes,
              recentTxCount: recentTxCount,
              avgTxCount: avgTxCount,
              blockInterval: avgBlockInterval,
              networkHashrate: 0 // To be computed if difficulty available
            };

...
// tests peer connectivity, returns boolean value
function testPortOpen(host, port = 9555, timeout = 500) {
  return new Promise((resolve) => {
    ...
    socket.once('connect', () => {
      ...
      setTimeout(() => {
        if (closedByPeer) {
          resolve(false); // port reacts, but does not offer handshake
        } else {
          socket.destroy();
          resolve(true);  // port really open
        }
      }, 50);
    });

    // timeout, refusal or error = port is closed or unreachable
    socket.once('timeout', () => {
      socket.destroy();
      resolve(false);
    });
    ...
    mongoose.connect(dbString).then(() => {
      if (database == 'index') {
          ...
          lib.get_networkinfo(function(info) {
            ...
            if (body != null) {
              ...
              async.timesSeries(body.length, function(i, loop) {
                ...
                // flag for inbound-only peer
                let is_inbound = body[i].inbound;
                ...
                isPeerReachable(address).then(is_open => {
                  ...
                  db.find_peer(address, port, function(peer) {
                    ...
                    if (peer) {
                      ...
                      // check if any peers should be saved
                      if (newPeers != null && newPeers.length) {
        ...
        if (settings.markets_page.enabled == true || settings.markets_page.market_price == 'COINGECKO') {
          ...
          } else if (settings.markets_page.market_price == 'COINGECKO' || settings.markets_page.market_price == 'COINPAPRIKA')
            get_market_price([]);
          else {
